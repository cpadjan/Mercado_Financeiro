...
|--------------------------------------------------------------|
|                                                              |
|       IMPORTA CNAB: ITAU, BRADESCO, SANTANDER E BV           |
|       DESTINO: BASE DE DADOS EM ACCESS                       |
|--------------------------------------------------------------|
| VERSÃO 2023-09-21                                            |
| TERMINAL: pip install pyodbc                                 |
|--------------------------------------------------------------|
'''
...
# PASTA DOS ARQUIVOS
path = r'...\Treasury\Bancos_CC\txt'
path_lido = r'...\Treasury\Bancos_CC\txt\lido'
...
def Layout_Itau_Linha_3 (linha):
    ...
    for linha in cnab:
        ...
        if cod_linha == '3':
            ...
            conn = pyodbc.connect(
                r'Driver={Microsoft Access Driver (*.mdb, *.accdb)};DBQ=...\Treasury\Bancos_CC\bd\2023_3_itau.accdb;')
            cursor = conn.cursor()
...
def Layout_Bradesco_Linha_3 (linha):
    ...
    for linha in cnab:
        ...
        if cod_linha == '3':
            ...
            conn = pyodbc.connect(
                r'Driver={Microsoft Access Driver (*.mdb, *.accdb)};DBQ=...\Treasury\Bancos_CC\bd\2023_3_bradesco.accdb;')
            cursor = conn.cursor()
...
def Layout_Santander_Linha_3 (linha):
    ...
    for linha in cnab:
        ...
        if cod_linha == '3':
            ...
            conn = pyodbc.connect(
                r'Driver={Microsoft Access Driver (*.mdb, *.accdb)};DBQ=...\Treasury\Bancos_CC\bd\2023_3_santander.accdb;')
            cursor = conn.cursor()
            sql = f"Insert into linha_3 (data_lancamento, c_c, historico1, historico2, nr_documento, valor_txt,  cod_lancamento, tipo, data_arq, hora_arq) VALUES ( '{data_lancamento}', '{c_c}', '{historico1}', '{historico2}','{nr_documento}' , '{valor_txt}', '{cod_lancamento}', '{tipo}', '{data_arq}', '{hora_arq}'  ) ;"
            cursor.execute(sql)
            cursor.commit()
...
def Layout_BV (linha):
    ...
    for linha in cnab:
        ...
        if cod_linha == '3':
            ...
            conn = pyodbc.connect(
                r'Driver={Microsoft Access Driver (*.mdb, *.accdb)};DBQ=...\Treasury\Bancos_CC\bd\2023_3_bv.accdb;')
            cursor = conn.cursor()
        ...
        if cod_linha == '1':
            ...
            conn = pyodbc.connect(
                r'Driver={Microsoft Access Driver (*.mdb, *.accdb)};DBQ=...\Treasury\Bancos_CC\bd\2023_1_bv.accdb;')
            cursor = conn.cursor()
            sql = f"Insert into linha_1 (data_lancamento, c_c, historico1, historico2, valor_txt,  cod_lancamento, tipo, data_arq, hora_arq) VALUES ( '{data_lancamento}', '{c_c}', '{historico1}', '{historico2}' , '{valor_txt}', '{cod_lancamento}', '{tipo}', '{data_arq}', '{hora_arq}'  ) ;"
            cursor.execute(sql)
            cursor.commit()
            cursor.close()
...
for arq in files:
    try:
        ...
        cnab.close()
        print("Arquivo não processado: " + arq)
